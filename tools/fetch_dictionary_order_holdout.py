...
out=Path('docs/dictionary-impact');old=Path('docs/addons-learning');original=json.loads((old/'wikipedia-expanded-holdout.json').read_text(encoding='utf-8'))
seen={m['pageid'] for r in original['records'] for m in r['members']}
seen.update(m['pageid'] for r in json.loads((old/'wikipedia-fresh-holdout.json').read_text(encoding='utf-8'))['records'] for m in r['members'])
records=[];errors=[]
...
for category in original['records']:
 ...
 params=urllib.parse.parse_qs(urllib.parse.urlparse(category['url']).query);params['cmlimit']=['150'];url='https://zh.wikipedia.org/w/api.php?'+urllib.parse.urlencode({k:v[0] for k,v in params.items()})
 try:
...
(out/'wikipedia-new-holdout.json').write_text(json.dumps(dict(role='new disjoint holdout after exact-match-first proposal; not used in compilation',records=records,errors=errors),ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
