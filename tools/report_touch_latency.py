...
OUT = ROOT/'docs/touch-latency'
rows = list(csv.DictReader((ROOT/'artifacts/touch-latency-corrected.tsv').open(), delimiter='\t'))
...
for source,target in [('touch-latency-corrected.tsv','samples.tsv.gz'),('touch-latency-rejected-ondraw.tsv','rejected-ondraw.tsv.gz')]:
    with gzip.open(OUT/target,'wb') as f:f.write((ROOT/'artifacts'/source).read_bytes())
(OUT/'summary.json').write_text(json.dumps(dict(baseline='5bcb90f / 0.7.6',samples=len(rows),groups=groups,
...
for key,value in groups.items():
    print(key, 'raw p95',round(value['editor_submission']['p95_ms'],2), 'candidates',value['candidate_submission'])
