...
for role in ['development','reserved']:
    before=load(role+'-before.tsv')
    for capacity in ([64,128] if role=='development' else [128]):
        after=load(f'{role}-capacity-{capacity}.tsv');reports[f'{role}-{capacity}']=compare(before,after)
...
(out/'capacity-results.json').write_bytes((json.dumps(reports,ensure_ascii=False,indent=2)+'\n').encode())
evidence=out/'evidence';evidence.mkdir(exist_ok=True)
for name in ['development-before.tsv','development-capacity-64.tsv','development-capacity-128.tsv','reserved-before.tsv','reserved-capacity-128.tsv','development-before.tsv.oracle.tsv']:
