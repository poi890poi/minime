param([string]$Output='artifacts/native-metadata/probe.exe',[switch]$SourceOnly)
...
try {
    ...
    if($SourceOnly) {
        ...
        $taskCommand='"{0}" >nul && "{1}" -S tools/desktop-native -B artifacts/native-source-only-clang -G Ninja "-DCMAKE_MAKE_PROGRAM={2}" "-DCMAKE_C_COMPILER={3}" "-DCMAKE_CXX_COMPILER={3}" -DCMAKE_BUILD_TYPE=Release && "{1}" --build artifacts/native-source-only-clang -j 6' -f $taskCompiler,$taskCmake,$taskNinja,$taskClang
        & cmd /d /c $taskCommand
