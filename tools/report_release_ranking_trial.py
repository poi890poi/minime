...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('before',type=Path);parser.add_argument('after',type=Path);parser.add_argument('output',type=Path);a=parser.parse_args()
    ...
    with a.before.open(encoding='utf-8') as left,a.after.open(encoding='utf-8') as right:
        for old,new in itertools.zip_longest(csv.DictReader(left,delimiter='\t'),csv.DictReader(right,delimiter='\t')):
            ...
            if x!=y or old['space']!=new['space']:changed.append(dict(before=old,after=new))
    report=dict(baseline='fd728d5',groups={k:dict(v) for k,v in sorted(groups.items())},hashes={str(p):hashlib.sha256(p.read_bytes()).hexdigest() for p in (a.before,a.after)})
