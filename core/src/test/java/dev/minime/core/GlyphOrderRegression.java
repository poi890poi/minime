...
final class GlyphOrderRegression {
    static void run()throws Exception {
        for(boolean englishOverlap:new boolean[]{false,true})for(String pack:Arrays.asList("taiwan","geography","japanese","poj")) {
            ...
            Editor nativeEditor=new Editor();engine=new CompositionEngine(nativeEditor,Learning.NONE);engine.dictionary(d);engine.start(false,false,false,false);
        ...
        CompositionEngine engine=new CompositionEngine(new Editor(),Learning.NONE);engine.dictionary(empty);engine.start(false,false,false,false);
