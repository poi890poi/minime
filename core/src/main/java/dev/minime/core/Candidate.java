...
public final class Candidate {
    ...
    private final boolean hideAlternate;
    ...
    private Candidate(String text,boolean literal,double score,String reading,int consumed,boolean supplemental,boolean abbreviated,boolean composed,boolean incomplete,PairedForms.Pair pair,String pack,boolean languageCharacter,boolean transliteration) {
        this(text,literal,score,reading,consumed,supplemental,abbreviated,composed,incomplete,pair,pack,languageCharacter,transliteration,false);
    }
    private Candidate(String text,boolean literal,double score,String reading,int consumed,boolean supplemental,boolean abbreviated,boolean composed,boolean incomplete,PairedForms.Pair pair,String pack,boolean languageCharacter,boolean transliteration,boolean hideAlternate) {
    ...
    Candidate asTransliteration() {return new Candidate(text,literal,score,reading,consumed,supplemental,abbreviated,composed,incomplete,pair,pack,languageCharacter,true,hideAlternate);}
    Candidate consuming(int count) {return new Candidate(text,literal,score,reading,count,supplemental,abbreviated,composed,incomplete,pair,pack,languageCharacter,transliteration,hideAlternate);}
    ...
    private Candidate pairedText(String value) {return text.equals(value)?this:new Candidate(value,literal,score,reading,consumed,supplemental,abbreviated,composed,incomplete,pair,pack,languageCharacter,transliteration,hideAlternate);}
    public String alternateText() {return pair==null || hideAlternate?"":text.equals(pair.phonetic)?pair.han:pair.phonetic;}
    /** Preserve lexical/learning identity while exposing only device-readable forms. */
    Candidate readable(java.util.function.Predicate<String> supported) {
        boolean primary=supported.test(text);
        String other=pair==null?"":text.equals(pair.phonetic)?pair.han:pair.phonetic;
        boolean secondary=!other.isEmpty() && supported.test(other);
        if(!primary && !secondary)return null;
        if(primary && (pair==null || secondary) && !hideAlternate)return this;
        return new Candidate(primary?text:other,literal,score,reading,consumed,supplemental,abbreviated,composed,incomplete,pair,pack,languageCharacter,transliteration,!primary || !secondary);
    }
